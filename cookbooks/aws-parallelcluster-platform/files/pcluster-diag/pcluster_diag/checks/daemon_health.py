...
class ClusterDaemonsAreRunning(Check):
    ...
    def _expected_daemons(self, context: Context):
        ...
        return NODE_TYPE_EXPECTED_DAEMONS.get(context.node_type.value, ())
    ...
    def run(self, context: Context) -> Result:
        ...
        if not_running:
            ...
            message = "Expected daemon(s) not running on the {}: {}.".format(context.node_type.value, details)
            return Result.failure(self, errors=[CheckError(self.DAEMONS_NOT_RUNNING, message)])
...
class ClustermgtdHeartbeatIsHealthy(Check):
    ...
    HEARTBEAT_READ_TIMED_OUT = "E1"  # reading the heartbeat file timed out (wedged filesystem)
    HEARTBEAT_UNREADABLE = "E2"  # the heartbeat file could not be read (missing / unavailable path)
    HEARTBEAT_STALE = "E3"  # the heartbeat is older than the stale threshold (clustermgtd wedged)
    ...
    def run(self, context: Context) -> Result:
        ...
        if read.timed_out:
            message = (
                ...
                "may be wedged (e.g. EBS/NFS stuck), which would also block clustermgtd.".format(
                    read.elapsed_seconds, path
                )
            )
            return Result.failure(self, errors=[CheckError(self.HEARTBEAT_READ_TIMED_OUT, message)])
        if read.returncode != 0:
            message = (
                "Could not read the clustermgtd heartbeat file {} (exit code {}). clustermgtd may never have "
                "started or the shared path is unavailable. stderr: {}".format(
                    path, read.returncode, read.stderr.strip()
                )
            )
            return Result.failure(self, errors=[CheckError(self.HEARTBEAT_UNREADABLE, message)])
        ...
        if age_seconds > CLUSTERMGTD_HEARTBEAT_STALE_THRESHOLD_SECONDS:
            message = (
                ...
                "self-terminate.".format(age_seconds, CLUSTERMGTD_HEARTBEAT_STALE_THRESHOLD_SECONDS)
            )
            return Result.failure(self, errors=[CheckError(self.HEARTBEAT_STALE, message)])
        return Result.passed(self)
