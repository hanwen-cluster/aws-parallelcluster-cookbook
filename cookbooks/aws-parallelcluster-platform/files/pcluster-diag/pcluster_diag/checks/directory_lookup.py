...
- ``DirectoryBackendIsReachable``: consults a read-only ``sssctl domain-status`` and fails when SSSD
  reports the directory backend offline (a fast lookup can be a cache hit, so it does not prove the
...
- ``DirectoryBindCredentialsAreValid``: reproduces SSSD's bind with a read-only ``ldapsearch`` to verify
  the configured bind DN and password (when the password is stored in plaintext).
...
# Per-probe classification tokens.
_STATUS_OK = "ok"
_STATUS_WARN = "warn"
_STATUS_FAIL = "fail"
...
class DirectoryLookupLatency(Check):
    ...
    # Error code carried by a FAILURE Result; the message lists the slow/failing lookups.
    SLOW_OR_FAILING_LOOKUPS = "E1"
    # Advisory code carried by a WARNING Result; the message lists elevated-latency or unresolved lookups.
    ELEVATED_OR_UNRESOLVED_LOOKUPS = "W1"
    ...
    def run(self, context: Context) -> Result:
        ...
        if failures:
            message = (
                "Slow or failing directory lookups detected: {}. This is the pattern that stalls slurmctld and can "
                "cause healthy nodes to be marked DOWN. Investigate the AD/LDAP endpoint, the network path, and "
                "SSSD tuning.".format("; ".join(failures))
            )
            return Result.failure(self, errors=[CheckError(self.SLOW_OR_FAILING_LOOKUPS, message)])

        warnings = [text for status, text in probes if status == _STATUS_WARN]
        if warnings:
            message = (
                "Elevated or unresolved directory lookups detected: {}. This has not reached the level that stalls "
                "slurmctld, but it is worth investigating the AD/LDAP endpoint, the network path, and SSSD tuning "
                "before it worsens under load.".format("; ".join(warnings))
            )
            return Result.warning(self, errors=[CheckError(self.ELEVATED_OR_UNRESOLVED_LOOKUPS, message)])
...
class DirectoryServiceManagedByClusterConfig(Check):
    ...
    # Advisory code carried by a WARNING Result.
    AD_NOT_MANAGED_BY_CLUSTER_CONFIG = "W1"
    ...
    def run(self, context: Context) -> Result:
        ...
        if _directory_service_config(context):
            return Result.passed(self)
        message = (
            "An Active Directory / LDAP integration is configured in {} but the cluster configuration has no "
            "DirectoryService section. The integration was set up outside ParallelCluster (e.g. via a custom "
            "bootstrap action), so ParallelCluster cannot manage or validate it and the configuration may be "
            "lost on a cluster update. Move the integration into the cluster configuration's DirectoryService "
            "section.".format(SSSD_CONF_PATH)
        )
        return Result.warning(self, errors=[CheckError(self.AD_NOT_MANAGED_BY_CLUSTER_CONFIG, message)])


class DirectoryLookupResiliencySettings(Check):
    """Warn when the settings that reduce directory-lookup load are not enabled."""

    # Advisory codes carried by a WARNING Result.
    NSS_SLURM_PLUGIN_DISABLED = "W1"
    SSSD_CACHE_CREDENTIALS_DISABLED = "W2"
    ...
    def run(self, context: Context) -> Result:
        ...
        if _nss_slurm_enabled(context) is False:
            advisories.append(
                CheckError(
                    self.NSS_SLURM_PLUGIN_DISABLED,
                    "The Slurm NSS plugin is not enabled ({} is absent from LaunchParameters in slurm.conf). "
                    "Without it, every user/group lookup on compute nodes goes to the directory backend instead "
                    "of being served locally by slurmd, multiplying directory load and lookup latency.".format(
                        NSS_SLURM_LAUNCH_PARAMETER
                    ),
                )
            )
        if not _cache_credentials_enabled():
            advisories.append(
                CheckError(
                    self.SSSD_CACHE_CREDENTIALS_DISABLED,
                    "SSSD credential caching is not enabled (cache_credentials is not set to True in {}). "
                    "Enabling it lets SSSD serve cached identities when the directory backend is slow or briefly "
                    "unavailable, reducing lookup latency and the risk of stalling slurmctld.".format(SSSD_CONF_PATH),
                )
            )
        if advisories:
            return Result.warning(self, errors=advisories)
        return Result.passed(self)
...
class DirectoryBackendIsReachable(Check):
    ...
    def run(self, context: Context) -> Result:
        ...
        if status.online is False:
            message = (
                "SSSD reports the directory backend offline: {}. User and group lookups are being served from "
                "the SSSD cache and will start failing once it expires, which can stall slurmctld and block "
                "logins. Investigate the AD/LDAP endpoint and the network path.".format(status.summary)
            )
            return Result.failure(self, errors=[CheckError(self.BACKEND_OFFLINE, message)])
        return Result.passed(self)
...
class DirectoryEndpointCertificateIsValid(Check):
    ...
    fail cluster-wide; with a relaxed ``reqcert`` (allow/never/try) SSSD proceeds unauthenticated, which
    is a latent security and stability risk. This check runs a read-only openssl TLS handshake.
    """

    INVALID_CERTIFICATE = "E1"  # cert fails to validate and reqcert is demand/hard -> SSSD will refuse
    UNTRUSTED_CERTIFICATE = "W1"  # cert fails to validate but reqcert is relaxed -> latent risk
    ...
    def run(self, context: Context) -> Result:
        """Fail (or warn, per ``reqcert``) when a directory endpoint's TLS certificate does not validate.
        ...
        validated = 0
        problems = []
        for host, port in ldaps_endpoints:
            try:
                ...
                return Result.skipped_not_applicable(self)
            verify_code = ldap.parse_tls_verify_code("{}\n{}".format(probe.stdout, probe.stderr))
            if probe.timed_out or verify_code is None:
                continue  # could not complete the handshake: a reachability concern, not a cert one
            validated += 1
            if verify_code != 0:
                problems.append("{}:{} (openssl verify code {})".format(host, port, verify_code))

        if not problems:
            return Result.passed(self) if validated else Result.skipped_not_applicable(self)

        # SSSD's reqcert default is "hard" when unset; hard/demand make an invalid cert fatal.
        hard = reqcert in ("", "hard", "demand")
        ...
        if hard:
            message = (
                ...
                "configured CA (ldap_tls_cacert).".format(detail, reqcert or "hard (default)")
            )
            return Result.failure(self, errors=[CheckError(self.INVALID_CERTIFICATE, message)])
        message = (
            "The directory endpoint TLS certificate did not validate: {}. ldap_tls_reqcert={} lets SSSD proceed "
            "anyway, but the connection is effectively unauthenticated; fix the certificate or the configured CA "
            "(ldap_tls_cacert).".format(detail, reqcert)
        )
        return Result.warning(self, errors=[CheckError(self.UNTRUSTED_CERTIFICATE, message)])
...
class DirectoryBindCredentialsAreValid(Check):
    ...
    INVALID_CREDENTIALS = "E1"  # the directory rejected the bind DN / password
    BIND_ERROR = "E2"  # the bind could not be completed (timeout or other ldapsearch error)
    ...
    def run(self, context: Context) -> Result:
        ...
        try:
            ...
            probe = ldap.ldap_bind_search(
                endpoints[0][0], bind_dn, password, base="", attributes=["1.1"], cacert=cacert, reqcert=reqcert
            )
        ...
        if probe.returncode == ldap.LDAP_INVALID_CREDENTIALS_CODE:
            message = (
                "The directory rejected SSSD's configured bind credentials (ldap_default_bind_dn / "
                "ldap_default_authtok). Identity lookups will fail until the bind DN and password are "
                "corrected (e.g. refresh the DirectoryService password secret)."
            )
            return Result.failure(self, errors=[CheckError(self.INVALID_CREDENTIALS, message)])
        detail = "timed out" if probe.timed_out else "exit code {}: {}".format(probe.returncode, probe.stderr.strip())
        message = (
            "Could not complete the directory bind to verify SSSD's credentials ({}). This may indicate the "
            "endpoint is unreachable or TLS is misconfigured.".format(detail)
        )
        return Result.failure(self, errors=[CheckError(self.BIND_ERROR, message)])
    ...
    def run(self, context: Context) -> Result:
        ...
        for user in users:
            ...
            if not probe.succeeded:
                # A bind/connectivity error is not a membership answer; leave it to the dedicated checks.
                return Result.skipped_not_applicable(self)
        ...
        if missing:
            message = (
                "Allow-listed user(s) not found under the configured LDAP search base '{}': {}. Those users "
                "cannot be resolved or log in even though the endpoint and credentials are healthy; the search "
                "base (ldap_search_base / ldap_user_search_base) is likely too narrow.".format(base, ", ".join(missing))
            )
            return Result.warning(self, errors=[CheckError(self.USER_NOT_UNDER_BASE, message)])
        return Result.passed(self)
...
def _ad_domain_names() -> List[str]:
    ...
    try:
        ...
        logger.warning("Could not read %s to determine SSSD domains: %s", SSSD_CONF_PATH, error)
        return []
    names = []
...
def _sssd_backend_status() -> Optional[_BackendStatus]:
    """Return a read-only ``sssctl domain-status`` snapshot of the AD/LDAP domain(s), or None.

    ``sssctl domain-status`` queries the running SSSD over its socket (read-only) and reports whether
    SSSD considers the backend online and which server it is using. This is best-effort: it never raises
    and returns None when sssctl is unavailable (not installed), errors, times out, or no AD/LDAP domain
    is configured.
    """
    summaries = []
    online_flags = []
    ...
    if False in online_flags:
        ...
        online = None
    return _BackendStatus(summary=" | ".join(summaries), online=online)
