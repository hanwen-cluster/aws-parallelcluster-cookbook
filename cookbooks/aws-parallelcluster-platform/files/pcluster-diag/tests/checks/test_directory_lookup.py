...
def test_run_warns_when_latency_between_thresholds(monkeypatch):
    ...
    assert [error.code for error in result.errors] == [DirectoryLookupLatency.ELEVATED_OR_UNRESOLVED_LOOKUPS]
    assert "Elevated or unresolved" in result.errors[0].message
...
def test_run_fails_when_latency_exceeds_fail_threshold(monkeypatch):
    ...
    assert [error.code for error in result.errors] == [DirectoryLookupLatency.SLOW_OR_FAILING_LOOKUPS]
    assert "stalls slurmctld" in result.errors[0].message
...
def test_run_warns_when_name_not_resolvable(monkeypatch):
    ...
    assert result.status is Status.WARNING
    assert [error.code for error in result.errors] == [DirectoryLookupLatency.ELEVATED_OR_UNRESOLVED_LOOKUPS]
    assert "no entry resolved" in result.errors[0].message
...
def test_managed_check_warns_when_ad_not_in_cluster_config(tmp_path):
    ...
    assert [error.code for error in result.errors] == [
        DirectoryServiceManagedByClusterConfig.AD_NOT_MANAGED_BY_CLUSTER_CONFIG
    ]
    assert "outside ParallelCluster" in result.errors[0].message
...
def test_resiliency_check_warns_when_nss_slurm_disabled(monkeypatch):
    ...
    assert result.status is Status.WARNING
    assert [error.code for error in result.errors] == [DirectoryLookupResiliencySettings.NSS_SLURM_PLUGIN_DISABLED]
...
def test_resiliency_check_warns_when_cache_credentials_disabled(monkeypatch):
    ...
    assert result.status is Status.WARNING
    assert [error.code for error in result.errors] == [
        DirectoryLookupResiliencySettings.SSSD_CACHE_CREDENTIALS_DISABLED
    ]
...
def test_resiliency_check_warns_with_both_advisories(monkeypatch):
    ...
    assert result.status is Status.WARNING
    assert [error.code for error in result.errors] == [
        DirectoryLookupResiliencySettings.NSS_SLURM_PLUGIN_DISABLED,
        DirectoryLookupResiliencySettings.SSSD_CACHE_CREDENTIALS_DISABLED,
    ]
...
def _backend(summary, online):
    """Build a _BackendStatus stub for monkeypatching _sssd_backend_status."""
    return directory_lookup._BackendStatus(summary=summary, online=online)
...
def test_backend_check_failures_when_backend_offline(monkeypatch):
    ...
    assert result.status is Status.FAILURE
    assert [error.code for error in result.errors] == [DirectoryBackendIsReachable.BACKEND_OFFLINE]
    assert "offline" in result.errors[0].message
    assert "cache" in result.errors[0].message
...
def test_cert_check_passes_when_certificate_validates(tmp_path, monkeypatch):
    _write_sssd(tmp_path, "[domain/default]\nid_provider = ldap\nldap_uri = ldaps://ad.example.com\n")
    monkeypatch.setattr(
        directory_lookup.ldap, "verify_tls_certificate", lambda *a, **k: _probe(0, "Verify return code: 0 (ok)")
    )
...
def test_cert_check_fails_when_invalid_and_reqcert_hard(tmp_path, monkeypatch):
    # reqcert unset defaults to hard => an invalid cert is fatal.
    _write_sssd(tmp_path, "[domain/default]\nid_provider = ldap\nldap_uri = ldaps://ad.example.com\n")
    monkeypatch.setattr(
        directory_lookup.ldap,
        "verify_tls_certificate",
        lambda *a, **k: _probe(1, "Verify return code: 21 (unable to verify the first certificate)"),
    )

    result = DirectoryEndpointCertificateIsValid().run(sample_context(NodeType.HEAD))

    assert result.status is Status.FAILURE
    assert [error.code for error in result.errors] == [DirectoryEndpointCertificateIsValid.INVALID_CERTIFICATE]
    assert "verify code 21" in result.errors[0].message


def test_cert_check_warns_when_invalid_but_reqcert_relaxed(tmp_path, monkeypatch):
    _write_sssd(
        tmp_path,
        "[domain/default]\nid_provider = ldap\nldap_uri = ldaps://ad.example.com\nldap_tls_reqcert = allow\n",
    )
    monkeypatch.setattr(
        directory_lookup.ldap, "verify_tls_certificate", lambda *a, **k: _probe(1, "Verify return code: 21 (bad)")
    )

    result = DirectoryEndpointCertificateIsValid().run(sample_context(NodeType.HEAD))

    assert result.status is Status.WARNING
    assert [error.code for error in result.errors] == [DirectoryEndpointCertificateIsValid.UNTRUSTED_CERTIFICATE]
...
def test_cert_check_skipped_when_no_handshake(tmp_path, monkeypatch):
    # A timeout / no verify code means the endpoint was unreachable: not this check's concern.
    _write_sssd(tmp_path, "[domain/default]\nid_provider = ldap\nldap_uri = ldaps://ad.example.com\n")
...
def test_bind_check_fails_on_invalid_credentials(tmp_path, monkeypatch):
    ...
    assert result.status is Status.FAILURE
    assert [error.code for error in result.errors] == [DirectoryBindCredentialsAreValid.INVALID_CREDENTIALS]
...
def test_bind_check_fails_on_other_bind_error(tmp_path, monkeypatch):
    ...
    assert result.status is Status.FAILURE
    assert [error.code for error in result.errors] == [DirectoryBindCredentialsAreValid.BIND_ERROR]
    assert "timed out" in result.errors[0].message
...
def test_membership_check_warns_when_user_missing(tmp_path, monkeypatch):
    ...
    assert result.status is Status.WARNING
    assert [error.code for error in result.errors] == [DirectoryUsersResolveUnderSearchBase.USER_NOT_UNDER_BASE]
    assert "bob" in result.errors[0].message and "alice" not in result.errors[0].message
