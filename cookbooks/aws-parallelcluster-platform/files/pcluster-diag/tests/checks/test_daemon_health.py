...
def test_daemons_reports_not_running_program_as_failure(monkeypatch):
    ...
    assert [error.code for error in result.errors] == [ClusterDaemonsAreRunning.DAEMONS_NOT_RUNNING]
...
def test_heartbeat_stale_fails_with_stuck_diagnosis(monkeypatch):
    ...
    assert result.status is Status.FAILURE
    assert [error.code for error in result.errors] == [ClustermgtdHeartbeatIsHealthy.HEARTBEAT_STALE]
...
def test_heartbeat_read_timeout_fails_pointing_at_wedged_filesystem(monkeypatch):
    ...
    assert result.status is Status.FAILURE
    assert [error.code for error in result.errors] == [ClustermgtdHeartbeatIsHealthy.HEARTBEAT_READ_TIMED_OUT]
...
def test_heartbeat_unreadable_file_fails(monkeypatch):
    ...
    assert result.status is Status.FAILURE
    assert [error.code for error in result.errors] == [ClustermgtdHeartbeatIsHealthy.HEARTBEAT_UNREADABLE]
